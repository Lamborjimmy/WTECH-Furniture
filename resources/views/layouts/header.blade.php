<style>
    ...
    <nav class="navbar navbar-expand-lg bg-light py-2 d-flex flex-column">
        <div class="container d-flex justify-content-between">
            <a href="{{ route('home') }}">
                <img src="{{ asset('assets/logo.png') }}" alt="Logo" style="max-width: 8.5rem;">
            </a>
            ...
            <div class="collapse navbar-collapse justify-content-between" id="navbar">
                ...
                <ul class="navbar-nav text-center d-flex align-items-center">
                    <li class="nav-item">
                        <a class="d-flex align-items-center gap-1 nav-link {{ request()->routeIs('home') ? 'active' : '' }}" href="{{ route('home') }}">
                            <i class="bi bi-house-fill"></i> Domov
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="d-flex align-items-center gap-1 nav-link {{ request()->routeIs('products.index') ? 'active' : '' }}" href="{{ route('products.index') }}">
                            <i class="bi bi-bag-fill"></i> Produkty
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="d-flex align-items-center gap-1 nav-link {{ request()->routeIs('login') ? 'active' : '' }}" href="{{ route('login') }}">
                            <i class="bi bi-person-fill"></i> Účet
                        </a>
                    </li>
                    <li class="nav-item">
                ...
                <hr class="d-lg-none">
                <ul class="navbar-nav text-center d-flex align-items-center d-lg-none mt-3">
                    @foreach ($placements as $placement)
                        <li class="nav-item">
                            <a class="d-flex align-items-center gap-1 nav-link" href="{{ route('products.index', ['placement' => $placement->id]) }}">
                                <i class="bi bi-{{ $placement->name == 'Spálňa' ? 'lamp-fill' : ($placement->name == 'Kuchyňa' ? 'cup-hot-fill' : ($placement->name == 'Kúpeľňa' ? 'droplet-fill' : ($placement->name == 'Obývacia izba' ? 'tv-fill' : 'laptop-fill'))) }}"></i>
                                {{ $placement->name }}
                            </a>
                        </li>
                    @endforeach
